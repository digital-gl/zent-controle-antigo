...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  ({ customClass, ...rest }, ref) => (
    <div
      ref={ref}
      {...rest}
      className={`absolute top-1/2 left-1/2 rounded-2xl border shadow-2xl [transform-style:preserve-3d] [will-change:transform] [backface-visibility:hidden] ${customClass ?? ""} ${rest.className ?? ""}`.trim()}
    />
  ),
);
...
export const CardSwap: React.FC<CardSwapProps> = ({
        ...
        if (stopAtIndex !== undefined && order.current[0] === stopAtIndex) {
          ...
          if (frontEl) {
            gsap.timeline({
              ...
              .to(frontEl, { y: -40, duration: 0.35, ease: "power2.out" })
              .to(frontEl, { y: 0, duration: 0.45, ease: "bounce.out" })
              .to(frontEl, { y: -28, duration: 0.3, ease: "power2.out" })
              .to(frontEl, { y: 0, duration: 0.4, ease: "bounce.out" });
          } else {
            window.setTimeout(() => {
              intervalRef.current = window.setInterval(swap, delay);
            }, delay);
  ...
  }, [cardDistance, verticalDistance, delay, pauseOnHover, skewAmount, easing, refs, stopAtIndex]);
